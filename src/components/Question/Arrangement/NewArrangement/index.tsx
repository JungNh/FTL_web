...
const NewArrangement: React.FC<Props> = ({
  lession,
  onNextLession,
  onSetResults,
  currentTestIndex,
  ...
  useEffect(() => {
    ...
    if (!_.isEmpty(lession)) {
      ...
      // console.log('NewArrangement', valueQ)
      const question: string = answerAPI?.value?.trim() || ''

      /**
       * @valueQ : Đáp án đúng từ API
       */
      const arrAnswer: { key: number; text: string }[] = question.includes('/')
  ...
  const onCheck = () => {
    ...
    if (
      ...
      correctString?.toUpperCase()?.replace(/\s/g, '')
    ) {
      setIsCorrect(true)
  ...
  const renderBody = () => {
    return (
      <div className="bg_renderBody">
        ...
        <p className="title__lession mb-5">{array || ''}</p>
        {lession?.hasImage && (
