...
import { cleanAndSplitSentence, cleanSentence, openError } from '../../../utils/common'
...
type QuesListType = { answer: string; videoUrl: string; question: string }

const BackTalkVideo: React.FC<Props> = ({
  backCourse,
  onNextLession,
  question,
  currentTestIndex,
  ...
  /**
   * Auto chạy quesiton audio and video
   */
  useEffect(() => {
    let timeOutObj: any
    if (quesList?.[currentRowIndex]?.videoUrl) {
      ...
      <div className="bg_renderBody">
        ...
        <div className="main__question">
            ...
            <Col xs={5}>
              <div>
                <ReactPlayer
                  ...
                  url={quesList?.[currentRowIndex]?.videoUrl}
                  playing={isPlaySound}
                />
              </div>
            </Col>
            <Col xs={6} className="conversation__container">
              {quesList.map((item: any, index: number) => (
                <React.Fragment key={index}>
                  {index < currentRowIndex && (
                    <React.Fragment key={index}>
                      <div className="message_left">
                        <Image className="avatar" src={ico_robot} />
          ...
          {isShowNext && (
            <Button.Shadow
              className="btn__next"
              content={currentRowIndex + 1 >= quesList.length ? 'Hoàn thành' : 'Tiếp tục'}
              onClick={() => {
                setIsShowNext(false)
                setCurrentRowIndex((curr) => curr + 1)
                if (currentRowIndex + 1 >= quesList.length) {
                  // sendAnswer()
                }
              }}
  ...
  return (
    <div className="lession__backTalkVideo">
      <Button.Shadow
