...
const SubTitleQues: FC<Props> = ({ question }) => {
  ...
  useEffect(() => {
    ...
    if (question?.type === 'arrangement') {
      setImgUrl((metasQuestion?.value) ? (metasQuestion.value) : (question?.imageUrl || null))
      setAudioUrl(question?.audioUrl || null)
    }
  ...
  const metasQuestion = (question as any)?.metas[0] as any

  return (
    <div>
      {(richText || audioUrl) && (
        <div>
          <p className="text_000">{question?.questionTitle}</p>
        </div>
      )}
      ...
      {metasQuestion !== undefined ? (
        <div className="my-3">
          {<Image className="quiz__image__question" src={convertUrl(imgUrl as string) || ''} />}
        </div>
      ) : (
        <div className="my-3">
          {/* <Image className="quiz__image__question" src={convertUrl('/image') || ''} /> */}
        </div>
      )}
      {audioUrl && (
