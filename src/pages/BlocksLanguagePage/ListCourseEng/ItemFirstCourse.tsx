...
const ItemFirstCourse: FC<Props> = ({
  ...
  return (
    <div className="english__child">
      <div className="bg_lang cursor" onClick={showMore}>
        <div className="title_course" style={{ color: COLOR.PRIMARY }}>
          {name}
        </div>
        <MdOutlineKeyboardArrowRight size={20} style={{ color: COLOR.PRIMARY }} />
      </div>
      {!_.isEmpty(data) ? (
          ...
          <AliceCarousel
            ...
            renderNextButton={() => (
              <FaChevronCircleRight
                className={`btn__right ${rightBtnDisabled || data?.length < 4 ? 'disabled' : ''}`}
              />
            )}
          >
            {data.map((item: any) => (
              ...
              >
                ...
                >
                  ...
                  <div className="text__course">
                    {item?.model !== 'contest' ? item.name : 'Luyện thi'}
                  </div>
                </div>
              </div>
            ))}
