...
const BasketBallGame: FC<Props> = ({ backCourse, onNextLession, lesson, highestScore }) => {
  ...
  const gameResources: GameResoucesType | undefined = useSelector((state: RootState) =>
    state.study.gameResouces?.find((i: GameResoucesType) => i.name === 'game_basketball')
  )
  ...
  useEffect(() => {
    dispatch(getGameResource({ name: 'game_basketball' }))
  }, [dispatch])
  ...
  const dataGame = useMemo(() => {
    const listQuestions = lesson?.questions?.map((question: any) => {
      ...
      return {
        ...
        answers,
        description: question?.questionExplain
      }
    })
  ...
  return (
    <div className="game__basketball">
      <GameMaxScreenOverlay />
      {loadingGame && <LoadingGame />}
      <Image
        className="game__background"
        src={screen === 'welcome' ? imageResources?.bg : imageResources?.bg_playing}
      />
      <Sound
